...
def remkBPNodeDir():
    dir = args.nodes_dir + 'bps/'
    run('rm -rf ' + dir)
    run('mkdir -p ' + dir)

def startBPNode(account):
    #  firstProducer, firstProducer + numProducers
    fp = firstProducer
    dir = args.nodes_dir + 'bps/'
    logFileName = "bps.log"
    otherOpts = ''.join(list(map(lambda i: ' --producer-name ' + account[fp+i]['name'] + ' --private-key \'["' + account[fp+i]['pub'] + '","' + account[fp+i]['pvt'] + '"]\'', range(numProducers))))
    otherOpts += (
        '    --plugin eosio::history_plugin'
        '    --plugin eosio::history_api_plugin'
    )
    ...
    cmd = (
        ...
        '    --data-dir ' + os.path.abspath(dir) +
        '    --chain-state-db-size-mb 1024'
        '    --http-server-address 127.0.0.1:' + str(8001) +
        '    --p2p-listen-endpoint 127.0.0.1:' + str(9001) +
...
parser.add_argument('--max-user-keys', metavar='', help="Maximum user keys to import into wallet", type=int, default=64)
parser.add_argument('--ram-funds', metavar='', help="How much funds for each user to spend on ram", type=float, default=0.1)
parser.add_argument('--min-stake', metavar='', help="Minimum stake before allocating unstaked funds", type=float, default=0.9)
parser.add_argument('--max-unstaked', metavar='', help="Maximum unstaked funds", type=float, default=50)
